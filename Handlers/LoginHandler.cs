...
public static class LoginHandler
{
    public static void Handle(byte[] data, TcpClient client)
    {
        try
        {
            ...
            // 🔥 Remove old session if exists (reconnect case)
            if (SessionManager.Sessions.ContainsKey(imei))
            {
                SessionManager.Sessions.TryRemove(imei, out _);
                Console.WriteLine($"Old session removed for: {imei}");
            }
            ...
            // 🔥 IMPORTANT: Store Client → IMEI mapping
            SessionManager.ClientMap[client] = imei;
